...
class mainAPP:
    def __init__(self):
        ...
        self.label = ttk.Label(self.root, text="title")
        self.label.pack()
        self.input_entry = ttk.Entry(self.root, width=30)
        self.input_entry.pack()
        ttk.Button(self.root, text="Find Cities",
                   command=self.changTitle).pack()
    ...
    def changTitle(self):
        input_gmt = self.input_entry.get()
        self.input_entry.delete(0, 'end')
        for place in gmt.items():
            if place[1] == int(input_gmt):
                self.label.config(text=showGMT(place))
                return
        self.label.config(text="Find no answer~~")
